...
class FluencyTutorMaterials extends React.Component {
  ...
  handleMaterialsAdd = async () => {
    ...
    if (!!file) {
      const doc = await axios.get("/api/fluency/video", { type: file.type });
      ...
      if (section === "w1v1") {
        await axios.post("/api/fluency/materials/w1v1",
          { desc: w1v1Desc, video: uploadConfigs.key });
      } else if (section === "w1v2") {
        await axios.post("/api/fluency/materials/w1v2",
          { desc: w1v2Desc, video: uploadConfigs.key });
      } else if (section === "w2v1") {
        await axios.post("/api/fluency/materials/w2v1",
          { desc: w2v1Desc, video: uploadConfigs.key });
      } else if (section === "w2v2") {
        await axios.post("/api/fluency/materials/w2v2",
  ...
  handleMaterialsDelete = async () => {
    ...
    if (section === "w1v1") {
      await axios.delete("/api/fluency/materials/w1v1");
    } else if (section === "w1v2") {
      await axios.delete("/api/fluency/materials/w1v2");
    } else if (section === "w2v1") {
      await axios.delete("/api/fluency/materials/w2v1");
    } else if (section === "w2v2") {
      await axios.delete("/api/fluency/materials/w2v2");
    }
    this.componentDidMount();
  };
